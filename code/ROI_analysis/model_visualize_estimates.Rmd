...
Predict parameter estimates from task conditions (target and domain) and age within parcel 66
## Linear effect of age, random intercepts
```{r}
model.1 = lmer(beta ~ target*domain*age_c + (1 | subjectID), data=filter(data.complete, parcellation == 66))
summary(model.1)
```

## Linear effect of age, random intercepts and slopes 
```{r}
model.2 = lmer(beta ~ target*domain*age_c + (1 + age_c | subjectID), data=filter(data.complete, parcellation == 66))
summary(model.2)
```

## Quadratic effect of age, random intercepts
```{r}
model.3 = lmer(beta ~ target*domain*age2_c + (1 | subjectID), data=filter(data.complete, parcellation == 66))
summary(model.3)
```
...
# Visualize raw data
## Plot LOESS curves for parcels 66 and 380
```{r}
# set color palette
...
## Plot fitted curves for parcels 66 and 380
```{r}
ggplot(data.complete, aes(x = age, 
                          y = beta, 
                          group = interaction(subjectID, target, domain), 
